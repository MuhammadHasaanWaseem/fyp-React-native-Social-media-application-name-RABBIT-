...
import { Text, Pressable, Alert, FlatList, SafeAreaView, Platform, Modal, TextInput, View as RNView, StyleSheet } from "react-native";
...
export default ({ user }: { user: User }) => {
  ...
  const filteredPosts = data?.filter((item) => {
    ...
    if (tab.name === Tab.CAPSULE) return item.status === 'capsule'; // Adjust for capsule logic if needed
    return false;
  }) || [];

  const followUser = async () => {
    try {
      const { error } = await supabase.from("Followers").insert({ user_id: userauth?.id, following_user_id: user?.id });
  ...
  const unfollowUser = async () => {
    try {
      const { error } = await supabase.from("Followers").delete().eq("user_id", userauth?.id).eq("following_user_id", user?.id);
  ...
  const handleBio = async (newBio: string, userId: string) => {
    try {
      const { error } = await supabase.from("User").update({ bio: newBio }).eq("id", userId);
  ...
  const handleNickname = async (newNickname: string, userId: string) => {
    try {
      const { error } = await supabase.from("User").update({ nickname: newNickname }).eq("id", userId);
      if (error) Alert.alert("Error", error.message);
  ...
  const openBioPrompt = () => {
    if (Platform.OS === "ios") {
      Alert.prompt("Edit Bio", "Enter your new bio:", (newBio) => { if (newBio !== undefined) handleBio(newBio, user.id); }, "plain-text", user?.bio || "");
    } else {
  ...
  const openNicknamePrompt = () => {
    if (Platform.OS === "ios") {
      Alert.prompt("Edit Nickname", "Enter your new nickname:", (newNickname) => { if (newNickname !== undefined) handleNickname(newNickname, user.id); }, "plain-text", user?.nickname || "");
    } else {
  ...
  const handleavatarupload = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        const { error: uploadError } = await supabase.storage.from("files").upload(filePath, { uri, name, type: mimeType }, { cacheControl: "3600", upsert: true });
        if (uploadError) {
          Alert.alert("Error", `Error uploading avatar: ${uploadError.message}`);
          return;
        }
        const { data: urlData, error: urlError } = supabase.storage.from("files").getPublicUrl(filePath);
        if (urlError) {
        ...
        const { error: updateError } = await supabase.from("User").update({ avatar: avatarUrlWithCacheBuster }).eq("id", user.id);
        if (updateError) Alert.alert("Error", `Error updating avatar: ${updateError.message}`);
        else {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#010118" }}>
      ...
      <ImageViewing images={[{ uri: localAvatar || user?.avatar }]} imageIndex={0} visible={isImageVisible} onRequestClose={() => setImageVisible(false)} />
          ...
          <AvatarGroup>
            {followers.slice(0, 3).map((item, index) => (
              <Avatar key={index} size="sm" className="border-1 border-outline-0">
                <AvatarFallbackText className="text-white">{item?.user?.username}</AvatarFallbackText>
      ...
      {isOwner ? (
        ...
        <HStack space="md" className="items-center justify-between p-6">
          <Button variant="outline" className="flex-1 rounded-xl" style={{ backgroundColor: isFollowing ? "black" : "white" }} onPress={isFollowing ? unfollowUser : followUser}>
            <ButtonText style={{ color: isFollowing ? "white" : "#141414", fontWeight: "900" }}>{followButtonText}</ButtonText>
          </Button>
      ...
      <HStack space="md" style={{ padding: 4 }}>
        {tabs.map((t) => (
          <Button key={t.name} variant="outline" className={`flex-1 rounded-xl ${t.name === tab.name ? "bg-white " : "bg-transparent"}`} onPress={() => setTab(t)}>
        ...
        {filteredPosts.length === 0 ? (
          <VStack style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ color: 'white', fontSize: 16 }}>
              {tab.name === Tab.PUBLIC ? "No public post" : tab.name === Tab.PRIVATE ? "No private post" : "No capsule post"}
            </Text>
          </VStack>
        ) : (
          <FlatList
      ...
      <Actionsheet isOpen={editSheet} onClose={handleClose}>
        ...
        <ActionsheetContent style={{ backgroundColor: "#141414" }}>
          ...
          <ActionsheetItem onPress={openBioPrompt}><ActionsheetIcon color="white" as={EditIcon} /><ActionsheetItemText style={{ color: "white" }}>Add/change Bio</ActionsheetItemText></ActionsheetItem>
          <Divider />
          <ActionsheetItem onPress={openNicknamePrompt}><ActionsheetIcon color="white" as={EditIcon} /><ActionsheetItemText style={{ color: "white" }}>Add Nickname</ActionsheetItemText></ActionsheetItem>
          <Divider />
          <ActionsheetItem onPress={handleavatarupload}><ActionsheetIcon color="white" as={DownloadIcon} /><ActionsheetItemText style={{ color: "white" }}>Upload Profile Picture</ActionsheetItemText></ActionsheetItem>
          <Divider />
          <ActionsheetItem onPress={handleClose}><ActionsheetIcon color="white" as={CloseCircleIcon} /><ActionsheetItemText style={{ color: "white" }}>Close</ActionsheetItemText></ActionsheetItem>
          <Divider />
        </ActionsheetContent>
      </Actionsheet>
      {Platform.OS !== "ios" && (
        <Modal transparent visible={showBioModal} animationType="slide">
          <RNView style={styles.modalContainer}>
            <RNView style={styles.modalContent}>
              ...
              <TextInput style={styles.input} placeholder="Enter your new bio" placeholderTextColor="#ccc" value={bioInput} onChangeText={setBioInput} />
              <HStack style={styles.modalButtons}>
                <Button variant="outline" onPress={() => setShowBioModal(false)}><ButtonText style={{ color: "white" }}>Cancel</ButtonText></Button>
                <Button variant="outline" onPress={() => { handleBio(bioInput, user.id); setShowBioModal(false); }}><ButtonText style={{ color: "white" }}>Submit</ButtonText></Button>
              </HStack>
            </RNView>
          </RNView>
        </Modal>
        ...
        <Modal transparent visible={showNicknameModal} animationType="slide">
          <RNView style={styles.modalContainer}>
            <RNView style={styles.modalContent}>
              ...
              <TextInput style={styles.input} placeholder="Enter your new nickname" placeholderTextColor="#ccc" value={nicknameInput} onChangeText={setNicknameInput} />
              <HStack style={styles.modalButtons}>
                <Button variant="outline" onPress={() => setShowNicknameModal(false)}><ButtonText style={{ color: "white" }}>Cancel</ButtonText></Button>
                <Button variant="outline" onPress={() => { handleNickname(nicknameInput, user.id); setShowNicknameModal(false); }}><ButtonText style={{ color: "white" }}>Submit</ButtonText></Button>
              </HStack>
            </RNView>
          </RNView>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  modalContent: { width: "80%", backgroundColor: "#141414", padding: 20, borderRadius: 10 },
  input: { borderColor: "#ccc", borderWidth: 1, borderRadius: 5, padding: 10, marginBottom: 20, color: "white" },
...
// } from "react-native";
// import { HStack } from "../ui/hstack";
// import PrivatePostView from "@/components/shared/PrivatePostView"; // private post
// import { Avatar, AvatarImage, AvatarFallbackText, AvatarGroup } from "../ui/avatar";
// import { Button, ButtonText } from "../ui/button";
// import { Divider } from "../ui/divider";
// import { useState } from "react";
...
// import { VStack } from "../ui/vstack";
// import { supabase } from "@/lib/supabase";
// import View from "@/components/shared/sharedview"; //public post
// import { usePosts } from "@/hooks/use-posts";
// import BottomSheet from "./bottom-sheet";
// import { useAuth } from "@/providers/AuthProviders";
// import { usefollowers } from "@/hooks/use-followers";
...
//             <>
//             {tab.name === Tab.PRIVATE && item.Availablity === 'private' ? (
//               <PrivatePostView item={item} refetch={refetch} />
//             ) : (
//               <View item={item} refetch={refetch} />
//             )}
